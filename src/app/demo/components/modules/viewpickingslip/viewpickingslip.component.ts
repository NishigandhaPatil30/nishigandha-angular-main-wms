...
@Component({
  selector: 'app-viewpickingslip',
  templateUrl: './viewpickingslip.component.html',
  styleUrls: ['./viewpickingslip.component.scss']
})
export class ViewpickingslipComponent implements OnInit {
  data: any[] = [];
  rowData: any;
  ...
  downloadPrint() {
    ...
    const phpEndpoint =
      'https://www.swatpro.co/get_picking_slip.php?ReferenceDoNo=' + this.referenceDoNo;
  ...
  printStickers(pickingslip: string, quantity: number) {
    const phpEndpoint =
      `https://www.swatpro.co/get_sticker_print.php?Pickingslip=${pickingslip}&Quantity=${quantity}`;
  ...
  fetchDataForReferenceDoNo(): void {
    ...
    const apiUrl = `https://www.swatpro.co/fetchviewpickingslip.php?ReferenceDoNo=${this.referenceDoNo}`;

...
@Component({
  selector: 'app-productwisestock',
  templateUrl: './productwisestock.component.html',
  styleUrls: ['./productwisestock.component.scss']
})
...
search(event: any): void {
  ...
  this.http.get(`https://swatpro.co/serachproductcodedepowise.php/?term=${term}&depot=${depot}`)
...
fetchDataForReferenceDoNo(DeliveryLocation: any, searchResults: any[]): void {
  const Productcode = DeliveryLocation.value.Productcode;
  const depot = this.depot;
  const apiUrl = `https://www.swatpro.co/productwisestockshowdepo.php?searchTerm=${encodeURIComponent(Productcode)}&depot=${encodeURIComponent(depot)}`;
  console.log(apiUrl);
...
exportToCSV(rowData: Data) {
  ...
  saveAs(blob, `productwisestock${rowData}.xlsx`);
}
...
exportToExcel() {
  ...
  saveAs(blob, 'productwisestock.xlsx');
}
}

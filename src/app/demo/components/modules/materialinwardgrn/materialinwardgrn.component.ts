...
@Component({
  selector: 'app-materialinwardgrn',
  templateUrl: './materialinwardgrn.component.html',
  styleUrls: ['./materialinwardgrn.component.scss'],
  providers:[MessageService]
})
export class MaterialinwardgrnComponent implements OnInit {
...
submitAllRows(i: number, Remark: any, Quantity: any, Productcode: any, Packing: any, Batch: any, Result: any, Brand: any, copiedQuantity: any, rowIndex: any) {
  ...
  for (let i = 0; i < this.rows.length; i++) {
   ...
   console.log(jsonData);
    const insertUrl = 'https://www.swatpro.co/wmsgrnentrytest1123warehosueadddepowise.php';
    this.http.post(insertUrl, jsonData).subscribe(
      (insertResponse) => {
        ...
        const grnno = insertResponse['Grnno'];
...
getProductBrand(Productcode: any, rowIndex: any) {
  const url = 'https://www.swatpro.co/inwardgetbranddepowise.php?Productcode=' + Productcode + '&Depot=' + this.depot;
  this.http.get(url)
    .subscribe((response: any) => {
      if (response.Brand) {
        this.rows[rowIndex].Brand = response.Brand;
      } else {
        this.messageService.add({severity:'warn', summary:'Warning', detail:'Product not found for the specified Depot.'});
        this.rows[rowIndex].Brand = null; 
        this.rows[rowIndex].Remark = null; 
        this.rows[rowIndex].Quantity = null; 
        this.rows[rowIndex].Invoicequantity = null; 
        this.rows[rowIndex].Weight = null; 
        this.rows[rowIndex].NetWeight = null; 
        this.rows[rowIndex].Batch = null; 
        this.rows[rowIndex].Expirydate = null; 
        this.rows[rowIndex].Manufacturingdates = null; 
        this.rows[rowIndex].InwardDate = null; 
      }
    });
    this.http.get('https://www.swatpro.co/inwardgetpackdepowise.php?Productcode=' + Productcode + '&Depot=' + this.depot)
    .subscribe((response: any) => {
      if (response.Packing) {
        this.rows[rowIndex].Packing = response.Packing;
      } else {
        this.messageService.add({severity:'warn', summary:'Warning', detail:'Packing not found for the specified Depot.'});
        this.rows[rowIndex].Packing=null;
      }
    });
    this.http.get('https://www.swatpro.co/inwardgetuomdepowise.php?Productcode=' + Productcode + '&Depot=' + this.depot)
    .subscribe((response: any) => {
      if (response.UOM) {
        this.rows[rowIndex].UOM = response.UOM;
      } else {
        this.messageService.add({severity:'warn', summary:'Warning', detail:'UOM not found for the specified Depot.'});
        this.rows[rowIndex].UOM =null;
      }
    });
    this.http.get('https://www.swatpro.co/inwardgethsncodedepowise.php?Productcode=' + Productcode + '&Depot=' + this.depot)
    .subscribe((response: any) => {
      if (response.Hsncode) {
        this.rows[rowIndex].Hsncode = response.Hsncode;
      } else {
        this.messageService.add({severity:'warn', summary:'Warning', detail:'Hsncode not found for the specified Depot.'});
        this.rows[rowIndex].Hsncode=null;
      }
    });
}
